...
namespace Microsoft.AspNetCore.Mvc.Internal
{
    ...
    public class ChunkingCookieManager
    {
                    ...
                    if (string.IsNullOrEmpty(chunk))
                    {
                        if (ThrowForPartialCookies)
                        {
                            ...
                            throw new FormatException(
                                string.Format(CultureInfo.CurrentCulture, "Incomplete chunked cookie", chunkId - 1, chunksCount, totalSize));
                        }
        ...
        public void DeleteCookie(HttpContext context, string key, CookieOptions options)
        {
            ...
            var existingValues = responseHeaders["Set-Cookie"];
            if (!StringValues.IsNullOrEmpty(existingValues))
            {
                responseHeaders["Set-Cookie"] = existingValues.Where(value => !rejectPredicate(value)).ToArray();
            }
